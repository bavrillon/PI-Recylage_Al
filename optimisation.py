...
def optimise_co2_with_scrap(id_site, id_alloy, id_scrap):
    ...
    for k in range(nb_elements) :  
        composition_raw_materials_and_scrap = [db.get_composition_raw_material(id_raw_mat) for id_raw_mat in raw_materials] + [db.get_composition_scrap(id_scrap)] # Composition in different elements of the raw materials + the scrap
...
def optimise_cost_with_scrap(id_site, id_alloy, id_scrap):
    ...
    cost_raw_materials_and_scrap = cost_raw_materials + [db.get_cost_scrap(id_site, id_scrap)]    # List of the cost of the raw materials + selected scrap
    raw_materials = db.get_raw_materials()    # List of the ID of the raw materials
    composition_ids = raw_materials + [id_scrap]   # List of the ID of the raw materials + the scrap to be mixed
    compostion_alloy_wished = db.get_composition_alloy(id_alloy)   
