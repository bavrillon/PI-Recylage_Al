...
def optimise_co2_with_scrap(id_site, id_alloy, id_scrap):
    ...
    compostion_alloy_wished = db.get_composition(id_alloy)   
    ...
    for k in range(len(composition_ids)) :  
        composition_raw_materials_and_scrap = [db.get_composition(id_raw_mat) for id_raw_mat in raw_materials].append(db.get_composition(id_scrap)) # Composition in different elements of the raw materials + the scrap
...
def optimise_co2_without_scrap(id_site, id_alloy):
    ...
    # Constraint of composition of the alloy :
    for k in range(len(composition_ids)) :  
        composition_raw_materials = [db.get_composition(id_raw_mat) for id_raw_mat in raw_materials]  # Composition in different elements of the raw materials 
